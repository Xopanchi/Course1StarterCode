...
public class EarthquakeCityMap extends PApplet {
	...
	private void selectMarkerIfHover(List<Marker> markers) {
		for (Marker marker : markers) {
			if (marker.isInside(map, mouseX, mouseY) && lastSelected == null) {
				...
				// System.out.println(" Pasamos por aquí
				// ---------------------------------------------------------------------------------------------------------");
				break;
			}
		}
	...
	/**
	 * Find one city inside de mouse click. If find one return true, and hide rest of cities.
	 * @return boolean.
	 */
	private boolean findAndShowOneCity() {
		// Find city and hide the others. Show quakes near city.
		boolean findFirstCity = false;
		for (Marker cityMarker : cityMarkers) {
			if (!findFirstCity && cityMarker.isInside(map, mouseX, mouseY)) {
				lastClicked = (CommonMarker) cityMarker;
				System.out.println("Find one city clicked");
				findFirstCity = true;
	...
	/**
	 * Find one quake inside de mouse click. If find one return true, and hide rest of quakes.
	 * @return
	 */
	private boolean findAndShowOneQuake() {
		boolean findFirstEarthQuake = false;
		for (Marker quakeMarker : quakeMarkers) {
			// Find earthquake and hide the others.
			if (!findFirstEarthQuake && quakeMarker.isInside(map, mouseX, mouseY)) {
				lastClicked = (CommonMarker) quakeMarker;
				System.out.println("Find one Earqueacke clicked");
				findFirstEarthQuake = true;
				quakeMarker.setHidden(false);
			} else {
				quakeMarker.setHidden(true);
			}
		}
		return findFirstEarthQuake;
	}
	...
	private void showCitiesInDanger() {
		double threatCircle = ((EarthquakeMarker) lastClicked).threatCircle();
		for (Marker cityMarker : cityMarkers) {
			if (lastClicked.getDistanceTo(cityMarker.getLocation()) <= threatCircle) {
				// Danger zone --> show city
				cityMarker.setHidden(false);
				System.out.println("Find city near quake");
			} else {
				cityMarker.setHidden(true);
			}
		}
	...
	private void showQuakesInVecinity() {
		for (Marker quakeMarker : quakeMarkers) {
			if (lastClicked.getDistanceTo(quakeMarker.getLocation()) <= ((EarthquakeMarker) quakeMarker)
					.threatCircle()) {
				// Danger zone --> show quake
				quakeMarker.setHidden(false);
				System.out.println("Find quake near city");
			} else {
				quakeMarker.setHidden(true);
			}
		}
